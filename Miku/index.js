...
app.get('/', (req, res) => res.send('Hello, im Miku UwU a bot made by Jssx#5283 if u need support join here: https://discord.gg/VhBD8XuSHh'))

app.listen(port, () =>
console.log(`Estamos preguntado a http://localhost:${port}`)
);
...
const mentionRegex = RegExp(`^<@!?${bot.user.id}>$`);
  ...
  if (message.content.match(mentionRegex)) {
    message.channel.send(
      ...
      .setDescription(`Oye! <@${message.author.id}>, Mi prefix es: \`\`\`${Prefix}\`\`\`Usa \`\`\`${Prefix}help\`\`\` O <@${bot.user.id}> help Para tener la lista de comandos`)
       .setColor("RANDOM")
  ...
  if(db.has(`afk-${message.author.id}+${message.guild.id}`)) {
        ...
        message.channel.send(`Bienvendi@ otra vez! ${message.author.username}, Feliz de verte!!`)
    }
    ...
    if(message.mentions.members.first()) {
        if(db.has(`afk-${message.mentions.members.first().id}+${message.guild.id}`)) {
          ...
          let time = db.fetch(`aftime-${message.mentions.members.first().id}+${message.guild.id}`);
				time = Date.now() - time;
           return message.channel.send(`**${message.mentions.members.first().user.username} esta afk - ${reason} - ${format(
						time
					)} ago**`);
        }
    }
    ...
    if(!message.content.startsWith(Prefix)) return;
     ...
     if (!message.guild.me.permissionsIn(message.channel).has("EMBED_LINKS"))
        return message.reply("**:x: necesito el permiso  `EMBED_LINKS`**");
...
bot.on("message", async message => {
    ...
    .setColor("#FF0000")
    .setDescription("Chat Bot desactivado!")
    .setFooter(`Requested by ${message.author.username}`)
    ...
    if(message.channel.id == chatbot.channel){
      ...
      if(message.attachments.size > 0)
        return message.channel.send("No puedo ver los logs :(\nPlease try to keep it in chat..")
     ...
     .setDescription(`Tu mensaje : **${message}**\nMy Message : **${data.cnt}**`)
     .setColor("RANDOM")
     .setFooter(`Hablando ${message.author.username}`, message.author.displayAvatarURL({
       dynamic: true
     }))
...
let roles = bot.setups.get(member.guild.id, "welcome.roles");
  ...
  if (!sMessage) sMessage = `Bienvendi@ al Server`;
