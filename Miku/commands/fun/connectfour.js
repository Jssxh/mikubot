...
module.exports = {
    config: {
        ...
        description: 'Juega un juego de conectar 4 cables',
    },
    run: async (bot, message, args, ops) => {
        ...
        if (current) return message.channel.send(`**Porfavor espera a que se tiermine el juego \`${current.name}\` TERMINADO**`);
        ...
        try {
            ...
            if (!verification) {
                ...
                return message.channel.send(`**Parece que.. ${opponent} No quiere jugar (esta cagado)**`);
            }
            ...
            while (!winner && board.some(row => row.includes(null))) {
                ...
                await message.channel.send(stripIndents`
					**${user}, Que columna quieres usar? Escribe \`end\` Para abandonar (si tienes miedo)**
					${displayBoard(board)}
					${nums.join('')}
				`);
                const filter = res => {
            ...
            ops.games.delete(message.channel.id);
            if (winner === 'time') return message.channel.send('**Juego terminado debido a pelea de viejos!**');
